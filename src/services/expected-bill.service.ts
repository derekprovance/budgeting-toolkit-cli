...
export class ExpectedBillService {
    ...
    async getExpectedBillSumForMonth(
        month: number,
        year: number,
    ): Promise<number> {
        try {
            ...
            for (const bill of activeBills) {
                if (this.isBillDueInMonth(bill, targetDate)) {
                    ...
                    logger.debug(
                        `Bill "${bill.name}" expected in ${month}/${year}: ${expectedAmount}`,
                        {
                            billId: bill.name,
            ...
            logger.info(
                `Total expected bills for ${month}/${year}: ${totalExpected}`,
                {
                    month,
                    year,
                    totalAmount: totalExpected,
                    billCount: activeBills.filter((bill) =>
                        this.isBillDueInMonth(bill, targetDate),
                    ).length,
                },
            );
    ...
    async getAverageMonthlyBillsForYear(year: number): Promise<number> {
        try {
            ...
            for (const bill of activeBills) {
                ...
                logger.debug(
                    `Bill "${bill.name}" yearly expected for ${year}: ${yearlyAmount}`,
                    {
                        billId: bill.name,
                        yearlyAmount,
                        frequency: bill.repeat_freq,
            ...
            logger.info(
                `Average monthly bills for ${year}: ${monthlyAverage}`,
                {
                    year,
    ...
    private async getActiveBills(): Promise<Bill[]> {
        try {
            ...
            const allBills = response.data.map(
                (billRead) => billRead.attributes,
            );
            ...
            logger.info(
                `Fetched ${activeBills.length} active bills (${allBills.length} total)`,
                {
    ...
    private isBillDueInMonth(bill: Bill, targetDate: Date): boolean {
        ...
        const billDate = new Date(
            bill.date + (bill.date.includes("T") ? "" : "T00:00:00Z"),
        );
        ...
        if (bill.end_date) {
            const endDate = new Date(
                bill.end_date +
                    (bill.end_date.includes("T") ? "" : "T00:00:00Z"),
            );
        ...
        if (billDate <= targetDate) {
            return this.calculateBillOccurrenceInMonth(
                bill,
                targetMonth,
                targetYear,
            );
    ...
    private calculateBillOccurrenceInMonth(
        bill: Bill,
        targetMonth: number,
        targetYear: number,
    ): boolean {
        ...
        switch (bill.repeat_freq) {
            ...
            case "monthly":
                ...
                const monthsSinceBillStart =
                    (targetYear - billYear) * 12 + (targetMonth - billMonth);
                const skipPattern = (bill.skip || 0) + 1; // skip=1 means every other month
                return (
                    monthsSinceBillStart >= 0 &&
                    monthsSinceBillStart % skipPattern === 0
                );

            case "quarterly":
                // Quarterly bills (every 3 months)
                const monthsSinceBillStartQuarterly =
                    (targetYear - billYear) * 12 + (targetMonth - billMonth);
                return (
                    monthsSinceBillStartQuarterly >= 0 &&
                    monthsSinceBillStartQuarterly % 3 === 0
                );

            case "half-year":
                // Half-yearly bills (every 6 months)
                const monthsSinceBillStartHalfYear =
                    (targetYear - billYear) * 12 + (targetMonth - billMonth);
                return (
                    monthsSinceBillStartHalfYear >= 0 &&
                    monthsSinceBillStartHalfYear % 6 === 0
                );
            ...
            default:
                logger.warn(
                    `Unknown repeat frequency: ${bill.repeat_freq} for bill "${bill.name}"`,
                );
                return false;
        }
    }
