...
const validateYear = (value: string): number => {
    ...
    if (isNaN(year) || year < 2000 || year > currentYear + 5) {
        throw new Error(
            `Year must be between 2000-${currentYear + 5}, got: ${value}`,
        );
    }
    return year;
};
...
Examples:
  ...
  $ budgeting-toolkit st                          # current month (using alias)`,
    ...
    program
        ...
        .alias("cat")
        .description(
            "AI-powered transaction categorization and budget assignment",
        )
        .addOption(
...
Examples:
  ...
  $ budgeting-toolkit cat Import-2025-06-23 -y                  # auto-apply changes`,
        ...
        .action(async (tag: string, opts: UpdateTransactionOptions) => {
            if (!tag || tag.trim() === "") {
                console.error(
                    "❌ Error: Tag parameter is required and cannot be empty",
                );
                console.log(
                    "\nUsage: budgeting-toolkit categorize <tag> [options]",
                );
                console.log(
                    "Example: budgeting-toolkit categorize Import-2025-06-23",
                );
                process.exit(1);
            }
            ...
            try {
                ...
                const command = new UpdateTransactionsCommand(
