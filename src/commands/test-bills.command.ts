...
export class BillsCommand implements Command<void, BudgetDateParams> {
    ...
    async execute(params: BudgetDateParams): Promise<void> {
        try {
            ...
            const monthlySum =
                await this.expectedBillService.getExpectedBillSumForMonth(
                    month,
                    year,
                );
            ...
            const yearlyAverage =
                await this.expectedBillService.getAverageMonthlyBillsForYear(
                    year,
                );

            // Display results
            console.log(
                `Expected bill sum for ${month}/${year}: $${monthlySum.toFixed(2)}`,
            );
            console.log(
                `Average monthly bills for ${year}: $${yearlyAverage.toFixed(2)}`,
            );
        } catch (error) {
            logger.error("Error calculating expected bills", {
                error,
                month: params.month,
                year: params.year,
            });
