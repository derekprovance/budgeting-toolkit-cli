...
- **First Split** (updates original transaction):
  - Copies category, budget, and tags from original transaction
  - User can optionally customize category/budget during split
  - Preserves transaction journal ID
- **Second Split** (creates new transaction):
  - Category, budget, and tags are intentionally left undefined
  - User must manually assign these fields in Firefly III
...
4. Optionally customize category/budget for each split
5. Preview split before confirmation
6. Confirm to execute the split
