...
async function seed() {
    ...
    console.log(`  ✓ Savings account: ${savingsId}`);
    console.log(`  ✓ Expense accounts: ${[groceriesExpId, diningExpId, netflixExpId, gasExpId, miscExpId].join(', ')}`);
    ...
    const entertBudgetRes = await client.budgets.storeBudget({ name: 'Entertainment', active: true });
