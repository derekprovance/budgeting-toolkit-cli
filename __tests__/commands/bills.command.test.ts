...
describe("BillsCommand", () => {
    ...
    describe("execute", () => {
        it("should display expected bills report for given month and year", async () => {
            ...
            mockExpectedBillService.getExpectedBillSumForMonth.mockResolvedValue(
                2855.93,
            );
            mockExpectedBillService.getAverageMonthlyBillsForYear.mockResolvedValue(
                2873.54,
            );
            ...
            // Assert
            expect(
                mockExpectedBillService.getExpectedBillSumForMonth,
            ).toHaveBeenCalledWith(6, 2024);
            expect(
                mockExpectedBillService.getAverageMonthlyBillsForYear,
            ).toHaveBeenCalledWith(2024);

            expect(consoleSpy).toHaveBeenCalledWith(
                "Expected Bills Report for 6/2024\n",
            );
            expect(consoleSpy).toHaveBeenCalledWith(
                "Expected bill sum for 6/2024: $2855.93",
            );
            expect(consoleSpy).toHaveBeenCalledWith(
                "Average monthly bills for 2024: $2873.54",
            );
        ...
        it("should handle service errors gracefully", async () => {
            ...
            mockExpectedBillService.getExpectedBillSumForMonth.mockRejectedValue(
                error,
            );
            ...
            expect(consoleErrorSpy).toHaveBeenCalledWith(
                "❌ Error calculating expected bills:",
                "Service error",
            );
        ...
        it("should format currency amounts correctly", async () => {
            ...
            // Assert
            expect(consoleSpy).toHaveBeenCalledWith(
                "Expected bill sum for 12/2023: $1234.57",
            );
            expect(consoleSpy).toHaveBeenCalledWith(
                "Average monthly bills for 2023: $9876.54",
            );
        ...
        it("should handle zero amounts", async () => {
            ...
            // Assert
            expect(consoleSpy).toHaveBeenCalledWith(
                "Expected bill sum for 1/2024: $0.00",
            );
            expect(consoleSpy).toHaveBeenCalledWith(
                "Average monthly bills for 2024: $0.00",
            );
