...
describe("PaycheckSurplusService", () => {
  ...
  beforeEach(() => {
    ...
    service = new PaycheckSurplusService(mockTransactionService, mockTransactionPropertyService);
  });
  ...
  describe("calculatePaycheckSurplus", () => {
    ...
    it("should calculate surplus when actual paychecks exceed expected amount", async () => {
      ...
      expect(logger.debug).toHaveBeenCalledWith(
        expect.objectContaining({
          month: 1,
          year: 2024,
          expectedPaycheckAmount: 5000.00,
          totalPaycheckAmount: 6000.00,
          surplus: 1000.00,
          paycheckCount: 2,
        }),
    ...
    it("should handle missing expected paycheck amount configuration", async () => {
      ...
      const { PaycheckSurplusService } = await import("../../src/services/paycheck-surplus.service");
      const testService = new PaycheckSurplusService(mockTransactionService, mockTransactionPropertyService);
    ...
    it("should handle API errors", async () => {
      // Arrange
      mockTransactionService.getTransactionsForMonth.mockRejectedValue(new Error("API Error"));
    ...
    it("should handle invalid month/year", async () => {
      ...
      // Act & Assert
      await expect(service.calculatePaycheckSurplus(13, 2024)).rejects.toThrow("Failed to find paychecks for month 13");
    });
  });
  ...
  describe("isPaycheck", () => {
    it("should identify valid paycheck transactions", () => {
      ...
      const result = (service as unknown as { isPaycheck: (t: TransactionSplit) => boolean }).isPaycheck(transaction);
