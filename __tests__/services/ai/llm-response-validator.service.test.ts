...
describe('LLMResponseValidator', () => {
    ...
    describe('validateCategoryResponse', () => {
        it('returns exact match', () => {
            expect(LLMResponseValidator.validateCategoryResponse('Coffee & Tea', categories)).toBe(
                'Coffee & Tea'
            );
        });
        it('returns fuzzy match', () => {
            expect(
                LLMResponseValidator.validateCategoryResponse('Coffee and Tea', categories)
            ).toBe('Coffee & Tea');
        });
        it('throws on invalid', () => {
            expect(() =>
                LLMResponseValidator.validateCategoryResponse('Invalid', categories)
            ).toThrow();
        });
        it('throws on empty', () => {
    ...
    describe('validateBudgetResponse', () => {
        it('returns exact match', () => {
            expect(LLMResponseValidator.validateBudgetResponse('Groceries', budgets)).toBe(
                'Groceries'
            );
        });
        it('returns fuzzy match', () => {
            expect(LLMResponseValidator.validateBudgetResponse('Grocerie', budgets)).toBe(
                'Groceries'
            );
        });
