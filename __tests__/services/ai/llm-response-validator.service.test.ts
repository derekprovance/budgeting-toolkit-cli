...
describe("LLMResponseValidator", () => {
    ...
    describe("validateCategoryResponse", () => {
        ...
        it("throws on empty", () => {
            expect(() =>
                LLMResponseValidator.validateCategoryResponse("", categories),
            ).toThrow();
        });
    });
    ...
    describe("validateBudgetResponse", () => {
        it("returns exact match", () => {
            expect(
                LLMResponseValidator.validateBudgetResponse(
                    "Groceries",
                    budgets,
                ),
            ).toBe("Groceries");
        ...
        it("returns empty string for empty", () => {
            expect(
                LLMResponseValidator.validateBudgetResponse("", budgets),
            ).toBe("");
        });
        it("throws on invalid", () => {
            expect(() =>
                LLMResponseValidator.validateBudgetResponse("Invalid", budgets),
            ).toThrow();
        });
    });
    ...
    describe("editDistance", () => {
        ...
        it("returns correct edit distance", () => {
            // @ts-expect-error Testing private method
            expect(LLMResponseValidator.editDistance("kitten", "sitting")).toBe(
                3,
            );
        });
    });
