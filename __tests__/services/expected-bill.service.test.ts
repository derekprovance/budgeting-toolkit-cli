...
describe("ExpectedBillService", () => {
  ...
  describe("getExpectedBillSumForMonth", () => {
    it("should validate month and year parameters", async () => {
      await expect(service.getExpectedBillSumForMonth(0, 2024)).rejects.toThrow();
      await expect(service.getExpectedBillSumForMonth(13, 2024)).rejects.toThrow();
      await expect(service.getExpectedBillSumForMonth(6, 24)).rejects.toThrow();
    });
    ...
    it("should only include active bills", async () => {
      const bills = [
        createMockBill({ name: "Active Bill", active: true, amount_max: "100.00" }),
        createMockBill({ name: "Inactive Bill", active: false, amount_max: "200.00" }),
      ];
    ...
    it("should handle yearly bills correctly", async () => {
      ...
      expect(await service.getExpectedBillSumForMonth(6, 2024)).toBe(1200);
      expect(await service.getExpectedBillSumForMonth(5, 2024)).toBe(0);
    ...
    it("should handle API errors", async () => {
      mockApiClient.get.mockRejectedValue(new FireflyApiError("API Error", 500));

      await expect(service.getExpectedBillSumForMonth(6, 2024)).rejects.toThrow(
  ...
  describe("getAverageMonthlyBillsForYear", () => {
    it("should validate year parameter", async () => {
      await expect(service.getAverageMonthlyBillsForYear(24)).rejects.toThrow();
    });
    ...
    it("should handle API errors", async () => {
      ...
      await expect(service.getAverageMonthlyBillsForYear(2024)).rejects.toThrow(
  ...
  describe("error handling", () => {
    it("should handle invalid amount formats", async () => {
      const bills = [
        createMockBill({ amount_max: "not-a-number" }),
      ];
      mockApiClient.get.mockResolvedValue(createMockBillResponse(bills));
