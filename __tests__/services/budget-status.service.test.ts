...
describe("BudgetStatusService", () => {
    ...
    describe("getBudgetStatus", () => {
        ...
        it("should throw error when API call fails", async () => {
            mockBudgetService.getBudgets.mockRejectedValueOnce(
                new Error("API Error"),
            );

            await expect(
                budgetStatusService.getBudgetStatus(1, 2024),
            ).rejects.toThrow("Failed to get budget status for month 1");
        });

        it("should validate month and year", async () => {
            await expect(
                budgetStatusService.getBudgetStatus(0, 2024),
            ).rejects.toThrow();
            await expect(
                budgetStatusService.getBudgetStatus(13, 2024),
            ).rejects.toThrow();
        });
    });
});
