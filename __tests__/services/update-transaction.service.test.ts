...
describe("UpdateTransactionService", () => {
  ...
  beforeEach(() => {
    ...
    mockTransactionService = {
      tagExists: jest.fn(),
      getTransactionsByTag: jest.fn().mockResolvedValue(mockTransactions as TransactionSplit[]),
    ...
    mockUpdater = {
      updateTransaction: jest.fn().mockImplementation((transaction, aiResults) => {
    ...
    (TransactionUpdaterService as jest.Mock).mockImplementation(() => mockUpdater);
    ...
    expect((service as unknown as { transactionService: TransactionService }).transactionService).toBe(mockTransactionService);
  ...
  describe("updateTransactionsByTag", () => {
    ...
    it("should process transactions and return HAS_RESULTS status", async () => {
      ...
      const filtered = (mockTransactions as TransactionSplit[]).filter((t) => mockValidator.shouldProcessTransaction(t, false));
    ...
    it("should handle successful updates", async () => {
      ...
      expect(mockValidator.shouldProcessTransaction).toHaveBeenCalledWith(mockTransaction, false);
