...
describe("DateUtils", () => {
  describe("validateMonthYear", () => {
    it("should not throw for valid month and year", () => {
      expect(() => DateUtils.validateMonthYear(4, 2024)).not.toThrow();
    });
  ...
  describe("validateBudgetDateParams", () => {
    it("should not throw for valid params", () => {
      const params: BudgetDateParams = { month: 4, year: 2024 };
      expect(() => DateUtils.validateBudgetDateParams(params)).not.toThrow();
    });

    it("should throw for invalid month", () => {
      const params: BudgetDateParams = { month: 13, year: 2024 };
    ...
    it("should throw for invalid year", () => {
      const params: BudgetDateParams = { month: 1, year: 1899 };
      expect(() => DateUtils.validateBudgetDateParams(params)).toThrow(
        "Year must be a valid 4-digit year"
      );
    });
